...
def request_email(change_request):
    ...
    subject = f"[{change_request['priority']}] AMP change request {change_request['requirement_signal_id']}"
    body = (
...
def forgot_password():
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            sent, _status = send_email(email, "Reset your AMP password", f"Reset your password within one hour:\n\n{reset_link}")
            if sent:
                reset_link = None
